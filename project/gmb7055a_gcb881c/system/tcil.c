...
//FAN2 left gmb7055 - 5200RPM *fan2 & fan4 use the same register
// int fun_T01020002()
// {
// 	int res = RESULT_FAIL;
// 	int passcnt = 0;
...
int fun_T01020003()
{
	...
	//calculate the fan RPM
	fan_rpm = (60000/(fan_rpm/10*2));
	//printf("fun_T01020003 fan_rpm = %.0f\r\n",fan_rpm);
	if( fan_rpm > 3800)
		passcnt ++;
	printf("passcnt = %d\r\n",passcnt);
	//fopen(ftest, "fan.txt", w);	
	//fprintf(ftest, "fun_T01020003 fan_rpm = %.0f\r\n", fan_rpm);
	fan_rpm = 0;
	//set half duty
	/*
	hw.u32_subtarget = FAN_HW_DUTY;
...
int fun_T01020004()
{
	...
	//calculate the fan RPM
	fan_rpm = (60000/(fan_rpm/10*2));
	//printf("fun_T01020004 fan_rpm = %.0f\r\n",fan_rpm);
	if( fan_rpm > 2800)
		passcnt ++;
	//printf("passcnt = %d\r\n",passcnt);
	//fprintf(ftest, "fun_T01020004 fan_rpm = %.0f\r\n", fan_rpm);
	fan_rpm = 0;
		sleep(2);
	//set half duty
	/*
...
*/	
	char buf[3];
	int i;
	system(" lspci -n | grep '1573:3800' > /root/diag/gcb881c/data/gpc_rev");
	FILE *fp=fopen("/root/diag/gcb881c/data/gpc_rev","r+");
