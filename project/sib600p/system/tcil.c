...
int init_tcil()
{
	...
	for(i=0;i<=13;i++)
	{
		...
		res = GPC_CTLCODE(GPC_CFG_PORT,ACTION_SET,&port);
		printf("Setting debounce of all intput ports,res= 0x%X\r\n", res);
	}
...
int fun_T01012004_00()
{	
	int ret,res = RESULT_FAIL;
	char buf[128];
	FILE *fp;

	ret = system("ifconfig | grep wlan > /root/diag/gmb7055/data/wlan_chk");
	fp = fopen("/root/diag/gmb7055/data/wlan_chk","r+");
	fgets(buf,10,fp);
...
// 	return uart_each_other_directio(0x240,0x3e8);//5 to 3
// 	//return uart_each_other_directio(0x240,0x2e8);
// 	//return uart_each_other_directio(0x3e8,0x280);
...
// 	//return 1;
// 	sleep(1);
// 	return uart_each_other_directio(0x248,0x2e8);//6 to 4
// 	//return uart_each_other_directio(0x248,0x3e8);
// 	//return uart_each_other_directio(0x2e8,0x288);
...
int fun_T01020002()
{
	...
	showjpeg(0,0,"./pics/800_600/system_fan.jpg");
	sleep(5);
...
/*
	//GPC 3800 Init input Port Access 20160314
	libport.u32_subtarget = PORT_ACTIVE;
	libport.u32_portnum = 0x3FFF;
	libport.u08_onoff = SET_ON;
	libport.u08_data = 0xFF;
	res = GPC_CTLCODE(GPC_CFG_PORT, ACTION_SET,&libport);
	printf("Enable all intput ports,res= 0x%X\r\n", res);
*/
	usleep(1000000);
//DK for interrut stress test 2016.08.08
	//usleep(900000);
	...
	GPC_CTLCODE(GPC_DATA_PORT,ACTION_SET,&libport);
	//DEBUGMSG("outputdata (0x%02X) to high (0x%02X)\r\n",out_portdata,libport.u08_data);
	//printf("outputdata (0x%02X) to high (0x%02X)\r\n",out_portdata,libport.u08_data);
	usleep(1000000);	
...
//DK for interrut stress test 2016.08.08
	...
	//printf("g_itrup=0x%x,g_itrup&itrbitval=0x%x\r\n",g_itrup,g_itrup&itrbitval);
	...
	GPC_CTLCODE(GPC_DATA_PORT,ACTION_SET,&libport);
	//DEBUGMSG("outputdata (0x%02X) to low (0x%02X)\r\n",out_portdata,libport.u08_data);
	//printf("outputdata (0x%02X) to low (0x%02X)\r\n",out_portdata,libport.u08_data);
	usleep(1000000);
...
//		usleep(900000);
	...
	//printf("g_itrdown=0x%x,g_itrdown&itrbitval=0x%x\r\n",g_itrdown,g_itrdown&itrbitval);
...
//ITR1 -> Port Q 6 //(0xfc->0xfe)
int fun_T02017026()
{	
	//return fun_TestITR('P',3,1,0x98,0x9c);
	return fun_TestITR_3800(21,6,1,1);
}
...
{	
	...
	return fun_TestITR_3800(21,7,2,2);
}
...
{	
	...
	return fun_TestITR_3800(20,4,3,3);
}
...
{	
	//return fun_TestITR('P',5,3,0x8c,0x9c);
	return fun_TestPort_3800(23,1,1,1);
}
//ITR4 -> Port Q 4
int fun_T02017032()
{	
	int res=RESULT_FAIL;
	//return fun_TestITR('P',5,3,0x8c,0x9c);
	res =  fun_TestPort_3800(23,0,1,2);
...
int fun_T02022002()	
{
	...
	showjpeg(0,0,"./pics/800_600/fan_led_green_red.jpg");
	sleep(4);
